...
int main(int argc, char** argv)
{
		...
		Heur_Table(argv[4]);   // Read the Heuristic Value.
...
void Dijkstra( int source, int dest)
{
    ...
    Temp_Node->g_n=0;
    Temp_Node->h_n=City_Hc[source];
    Temp_Node->f_n=City_Hc[source];
    Temp_Node->d=0;
  ...
  //  Node_Generated++;
	...
	for(index=rear+1;index<front;index++)	
    	{
	    		...
	    		printf("\n");
		    do{
				printf("< state = %s, g(n)= %d, d = %d  Parent : ",City_list[Temp->Node],Temp->d,Temp->f_n);	
    ...
    while(Generated_Node[rear]->Node!=dest)
    {
    	...
    	Temp=Generated_Node[rear];	
   		printf("\n");
	    do{
	    		printf("< state = %s, g(n)= %d, d = %d f(n)= %d Parent : ",City_list[Temp->Node],Temp->g_n,Temp->d,Temp->f_n);
				Temp=Temp->Parent_Node;	
    	}while(Temp!=NULL);
		...
		for(index=0;index<Number_of_Vertex;index++)	
    	{
	    	if(adjacent_matrix[Generated_Node[rear]->Node][index]!=0)
	    	{
    			...
    			Temp_Node->g_n=adjacent_matrix[Generated_Node[rear]->Node][index]+Generated_Node[rear]->g_n;
   			    Temp_Node->h_n=City_Hc[index];
    			Temp_Node->f_n=Temp_Node->g_n+Temp_Node->h_n;
...
{
	FILE *fp;
    int i,i1,i2,flag,city_pos,Counter,origin,destin, wt;
    char city[255],h_cost[255],Str_f[200];
	
	if((fp=fopen(H_file, "r"))==NULL) //Reading from Input File.
	{
		printf("Sorry File %s can not open",H_file);     
		{getch();exit(0);}
	}
	for(i=0;;i++)
	{
		// Read the Heuristic cost of the City.
		fscanf(fp, "%s", city);
		if (feof(fp))
			 break;	
	     	
		fscanf(fp, "%s", h_cost); //Reading weight for a pair of node.
		wt=atoi(h_cost);
	
		Counter=0;flag=0;
		while(Counter<Number_of_Vertex)
		{
			if(!(strcmp(City_list[Counter],city)))
			{
				flag=1;city_pos=Counter;
				break;
			}	
			Counter++;
		}
...
/*
	printf("\n");
	for(i1=0;i1<i;i1++)
	printf("[%s] %d ",City_list[i1],City_Hc[i1]);
*/
	fclose(fp);
}
//End of Heur_Table()